  ...
  return (
    <section className="container flex gap-2.5 lg:grid lg:grid-cols-4 lg:gap-4 mb-10 overflow-x-auto scrollbar-hide snap-x snap-mandatory ">
      {categories.map((category, index) => (
        <div
          key={index}
          className="relative h-125 bg-cover bg-center rounded-[20px] flex items-center justify-center text-white shrink-0 w-[95%] md:w-1/2 lg:w-full"
          style={{ backgroundImage: `url(${category.image})` }}
        >
          <div className="absolute inset-0 bg-black/30 rounded-[20px] snap-center" />
